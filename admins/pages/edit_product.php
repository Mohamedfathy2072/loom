    ...
    if(isset($_POST['btn'])):
      // here filter data
        ...
        if(empty($title) || empty($price) || empty($categorys) || empty($description)):
          ...
          // here edit data
          $edit = $conn->prepare("UPDATE product SET title = :title,price = :price,category=:category,size_title=:size_title,size=:size,description=:description,price_coupon = :price_coupon,quantity = :quantity,rt_product = :rt_product,grenti = :grenti WHERE ID = :id");
