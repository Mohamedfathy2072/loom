      ...
      if(isset($_POST['btn'])):
        // here filter data
          ...
          if(empty($title) || empty($price) || empty($categorys) || empty($description) || empty($imageType1) || empty($imageFile1)):
            ...
            // here insert data
            $insert = $conn->prepare("INSERT INTO product(title,price,category,size_title,size,description,img_type,img_file,price_coupon,quantity,rt_product,grenti,generate_code) VALUES(:title,:price,:category,:size_title,:size,:description,:img_type,:img_file,:price_coupon,:quantity,:rt_product,:grenti,:generate_code)");
