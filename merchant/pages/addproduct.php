<?php
      ...
      if(isset($_POST['btn'])):
        // here filter data
          ...
          if(empty($title) || empty($price) || empty($categorys) || empty($description) || empty($imageType1) || empty($imageFile1)):
            ...
            // here insert data
            $insert = $conn->prepare("INSERT INTO product_merchant(title,price,category,size_title,size,description,description_api,img_type,img_file,status_product,price_coupon,quantity,rt_product,grenti,generate_code,id_user) VALUES(:title,:price,:category,:size_title,:size,:description,:description_api,:img_type,:img_file,'في الانتظار',:price_coupon,:quantity,:rt_product,:grenti,:generate_code,:id_user)");
            ...
            if($insert->execute()){
              // here add more image
              $product = $conn->query("SELECT * FROM `product_merchant` ORDER BY ID DESC LIMIT 1 OFFSET 0");
      ...
      }catch (PDOException $error_packge){
        echo "رجاء لا ترفع ملف حجمها اكثر من 40 ميجا بايت";
      }
    ?>
