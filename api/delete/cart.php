            ...
            if($method == "POST"):
                ...
                if(isset($_POST['email']) && !empty($_POST['email']) && isset($_POST['password']) && !empty($_POST['password']) && isset($_POST['id_product']) && !empty($_POST['id_product'])){
                    ...
                    if($login->rowCount() == 1){
                        ...
                        // here check cart data with table isset or no
                        $cart_data = $conn->prepare("SELECT * FROM cart WHERE iD = :id_product AND member_id = :user_id");
                        $cart_data->bindParam("id_product",$id_product);
                        $cart_data->bindParam("user_id",$data['id']);
                        $cart_data->execute();

                        if($cart_data->rowCount() == 1){

                            // here if delete cart with database
                            if($cart_data->execute()){

                            // here delete data with cart table
                            $cart_data = $conn->prepare("DELETE FROM cart WHERE iD = :id_product AND member_id = :user_id");
                            $cart_data->bindParam("id_product",$id_product);
                            $cart_data->bindParam("user_id",$data['id']);
                            if($cart_data->execute()){
                                ...
                                $product_info = array(
                                    "login"=>true,
                                    "status"=>true,
                                    "data"=>"تم الحذف من عربة التسويق بنجاح"
                                );
                            ...
                            header("HTTP/1.1 404 Not Found");
                            // here if table cart empty
                            $product_info = array(
                                    "login"=>true,
                                    "status"=>false,
                                    "data"=>"غير موجود بيانات بداخل عربة التسويق بهذا اي دي معلومات"
                                );
