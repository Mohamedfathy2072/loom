            ...
            if($method == "POST"):
                ...
                if(isset($_POST['email']) && !empty($_POST['email']) && isset($_POST['password']) && !empty($_POST['password']) && isset($_POST['id_product']) && !empty($_POST['id_product'])){
                    ...
                    if($login->rowCount() == 1){
                        ...
                        $cart_data = $conn->prepare("INSERT INTO cart(id_product_card,member_id) VALUES(:id_product,:user_id)");
                        ...
                        if($cart_data->execute()){
                            ...
                            $product_info = array(
                                "login"=>true,
                                "status"=>true,
                                "data"=>"تم الاضافة الي عربة التسويق بنجاح"
                            );
                            ...
                            $product_info = array(
                                "login"=>true,
                                "status"=>false,
                                "data"=>"لم بتم اضافة الي عربة التسويق بنجاح"
                            );
